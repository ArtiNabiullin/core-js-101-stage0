...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const start1 = isStartIncluded ? '[' : '(';
  const end1 = isEndIncluded ? ']' : ')';
  return `${start1}${start}, ${end}${end1}`;
}
...
function isCreditCardNumber(ccn) {
  const digits = String(ccn).split('').reverse();
...
function evaluateTicTacToePosition(position) {
  const check = (a, b, c) => {
    return a && a === b && a === c ? a : undefined;
  };

  // rows
  for (let i = 0; i < 3; i += 1) {
    const winner = check(position[i][0], position[i][1], position[i][2]);
    if (winner) return winner;
  }

  // columns
  for (let i = 0; i < 3; i += 1) {
    const winner = check(position[0][i], position[1][i], position[2][i]);
    if (winner) return winner;
  }

  // diagonals
  return (
    check(position[0][0], position[1][1], position[2][2]) ||
    check(position[0][2], position[1][1], position[2][0]) ||
    undefined
  );
}
